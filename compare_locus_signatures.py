...
def is_shell_protein(protein_pfam):
    if ("P_" in protein_pfam) or ("H_" in protein_pfam) or ("T_" in protein_pfam) or ("Hp_" in protein_pfam) \
            or ("Ts_" in protein_pfam) or ("Tsp_" in protein_pfam) or ("Tdp" in protein_pfam):
        return True
    else:
        return False
    ...
    for protein1 in locus1:
        ...
        for protein2 in locus2:
            ...
            if (protein1_pfam == protein2_pfam) and (protein1_pfam != "#"):
                ...
                if not is_shell_protein(
                        protein1_pfam):  # only add weight from surrounding shell proteins to non-shell proteins
                    for test_surrounding in range(protein1_position - shell_protein_nearby_range,
                                                  protein1_position + shell_protein_nearby_range):
                        ...
                        if 0 < test_surrounding < len(locus1) - 1:
    ...
    for protein1 in locus1_list:
        if protein1 != "#":
            if protein1 in locus2_list:
                ...
                counter2 = 1
                try:
                    while locus1_list[locus1_match_position + counter1] == \
                            locus2_list[locus2_match_position + counter2]:
                        match += 1
                        if match > max_match:
                            max_match = match
                        counter1 += 1
                ...
                try:
                    while (locus1_list[locus1_match_position + counter1] ==
                           locus2_list[locus2_match_position + counter2]) and (locus2_match_position + counter2 > -1):
                        match += 1
                        if match > max_match:
                            max_match = match
                        counter1 += 1
                ...
                try:
                    while (locus1_list[locus1_match_position + counter1] ==
                           locus2_list[locus2_match_position + counter2]) and (locus1_match_position + counter1 > -1):
                        match += 1
                        if match > max_match:
                            max_match = match
                        counter1 += -1
                ...
                try:
                    while (locus1_list[locus1_match_position + counter1] ==
                           locus2_list[locus2_match_position + counter2]) and (locus1_match_position + counter1 > -1) \
                            and (locus2_match_position + counter2 > -1):
                        match += 1
                        if match > max_match:
                            max_match = match
                        counter1 += -1
    ...
    for i in range(len(locus_collection)):
        ...
        for topx in range(0, report_max_scores):
            try:
                ...
                print('\t'.join(map(str,
                                    (locus_collection[i][0], locus_collection[top_id][0], total_score,
                                     locus_collection[i][1], locus_collection[top_id][1]))))
            except IndexError:
                pass
